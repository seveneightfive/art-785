...
export const getArtistBySlug = async (slug) => {
  const { data, error } = await supabase.from('Artists').select('*, reviews(*), events(*)').eq('slug', slug).single();
...
export const getEventBySlug = async (slug) => {
  ...
  if (eventData && eventData.artist_ids) {
    const { data: artistsData, error: artistsError } = await supabase.from('Artists').select('*').eq('id', eventData.artist_ids);
    if (artistsError) {
